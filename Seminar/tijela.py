...
class Tijelo:
    ...
    def __korak(d_sudar,t,dt):
        ...
        """for obj in Tijelo.objekti:
            for meta in Tijelo.objekti:
                if (obj.ime == "KOMET" and meta.ime == "Merkur") or (obj.ime == "Merkur" and meta.ime == "Komet"):
                    Tijelo.d.append(mt.sqrt((obj.r[-1][0]-meta.r[-1][0])**2+(obj.r[-1][1]-meta.r[-1][1])**2))"""
    ...
    def gibanje(t_fin,dt):
        ...
        t=0
        while t<t_fin:
            Tijelo.__korak(3.5e8,t,dt)
            t+=dt

        crtaj_svakih_n=2 #Izgleda optimalno, crta svaku 2. tocku
...
Komet = Tijelo(1e23,[-4.5*149.6e9,5.791e10],[17.050435e3,2200], "KOMET") #Valja se igrat sa masom  kometa, da se vidi sudar
Tijelo.gibanje(365.242*24*60*60*2, 3600*6) #2 godine
